...
모아를 열면 서버가 마지막 실행 시각을 확인하여 주제별로 하루 한 번 검색합니다. 사용자가 `지금 새 자료 찾기`를 누르면 즉시 다시 검색합니다. 후보는 제목과 요약의 키워드를 이용한 규칙 기반 점수로 정렬하며, 현재 자동 수집함에는 관련도 70점 이상만 표시합니다.
...
6. 사용자가 선택한 후보만 정식 자료함의 `sources`로 저장합니다.
