...
export default function Home() {
  ...
  const [documentType, setDocumentType] = useState<"기획서" | "보고서" | "빈 문서">("기획서");
  ...
  function startQuestion(value: string) {
    setQuery(value);
    setActive("chat");
    setAnswer(`“${value}” 질문은 ChatGPT에서 모아 앱을 활성화한 뒤 그대로 물어보세요. ChatGPT가 저장된 자료를 검색하고 원문을 확인해 출처와 함께 답합니다.`);
  }

  async function createDocumentPrompt() {
    const brief = documentBrief.trim();
    if (!brief) {
      setNotice("문서의 목적과 독자를 먼저 적어주세요.");
      return;
    }
    const prompt = `모아에 저장된 자료를 검색하고 원문을 확인한 뒤 ${documentType} 초안을 작성해줘.\n\n목적과 독자: ${brief}\n\n요구사항:\n- 근거가 되는 자료의 제목과 출처를 표시할 것\n- 자료에 없는 내용은 추론 또는 제안으로 구분할 것\n- 바로 편집할 수 있는 완성된 초안 형태로 작성할 것`;
    await navigator.clipboard.writeText(prompt);
    setNotice("ChatGPT에 붙여넣을 문서 요청문을 복사했습니다.");
  }
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="filters">
          <button className={filterKind === "DOCUMENT" ? "selected" : ""} onClick={() => { setFilterKind("DOCUMENT"); setActive("library"); setShowAll(true); }}>문서 <b>{counts.documents}</b></button>
          <button className={filterKind === "WEB" ? "selected" : ""} onClick={() => { setFilterKind("WEB"); setActive("library"); setShowAll(true); }}>웹페이지 <b>{counts.web}</b></button>
          <button className={filterKind === "MEMO" ? "selected" : ""} onClick={() => { setFilterKind("MEMO"); setActive("library"); setShowAll(true); }}>메모 <b>{counts.memo}</b></button>
        </div>
      ...
      <section className="workspace">
        <header><div><h1>{active === "library" ? "내 자료함" : active === "chat" ? "자료에게 질문" : "문서 작업실"}</h1><p>{active === "library" ? "흩어진 자료를 모으고, 연결하고, 새로운 생각으로 발전시키세요." : active === "chat" ? "저장한 자료를 근거로 답을 찾습니다." : "자료를 바탕으로 기획서와 보고서의 초안을 만듭니다."}</p></div><button className="help" onClick={() => setModal("help")} aria-label="사용 방법">?</button></header>
        ...
        {active === "chat" && <section className="chat-panel">
          ...
          <div className="prompts"><button onClick={() => startQuestion("모아의 최근 자료에서 핵심 인사이트를 정리해줘")}>최근 자료 핵심 인사이트</button><button onClick={() => startQuestion("모아 자료를 근거로 새 기획의 기회 영역을 찾아줘")}>기획 기회 영역 찾기</button></div>
        </section>}
        ...
        {active === "studio" && <section className="studio">
          ...
          <div className="templates"><button className={documentType === "기획서" ? "selected" : ""} onClick={() => setDocumentType("기획서")}><span>▤</span><strong>기획서</strong><small>배경부터 실행 계획까지</small></button><button className={documentType === "보고서" ? "selected" : ""} onClick={() => setDocumentType("보고서")}><span>▥</span><strong>보고서</strong><small>핵심 요약과 근거 중심</small></button><button className={documentType === "빈 문서" ? "selected" : ""} onClick={() => setDocumentType("빈 문서")}><span>◇</span><strong>빈 문서</strong><small>자유롭게 시작하기</small></button></div>
          <textarea value={documentBrief} onChange={(event) => setDocumentBrief(event.target.value)} placeholder="만들고 싶은 문서의 목적과 독자를 적어주세요. 예: 경영진 공유용 신규 서비스 기획서" />
          {notice && <p className="studio-notice">{notice}</p>}
          <button className="create-doc" onClick={createDocumentPrompt}>ChatGPT 요청문 복사 <span>→</span></button>
        </section>}
      ...
      {modal && <div className="modal-backdrop" onMouseDown={() => setModal(null)}><div className="modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {modal === "help" ? <><h2>모아 사용 방법</h2><p>자료를 저장한 뒤 ChatGPT에서 모아 앱을 켜고 질문하세요.</p><div className="help-steps"><span><b>1</b> 파일·링크·메모를 자료함에 저장</span><span><b>2</b> 자료 카드를 눌러 추출된 원문 확인</span><span><b>3</b> ChatGPT에서 모아 앱을 활성화</span><span><b>4</b> 출처를 요청하며 질문하거나 문서 작성</span></div></> :
